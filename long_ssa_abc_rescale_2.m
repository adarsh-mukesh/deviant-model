...
t_Sim=2000*step;% in seconds ; total simulation time seconds (to be put in same units)
...
for step_num=1:step:t_Sim
    ...
    for t=2:step/dt
        for i=1:thal
            ...
            x_thal(i,:,t)=depressing_synapses(x_thal(i,:,t-1),Tre_thal,Tei_thal,tir,del);
        end
        for i=1:sp
            ...
            if ((v_sp(i,t)>=v_th_sp) || (rand<=1-exp(-1*fs*dt)))
                sp_last_spk(i,1)=t;
            ...
            x_sp(i,:,t)=depressing_synapses(x_sp(i,:,t-1),Tre_sp,Tei_sp,tir,del);
            a=sp_fire_time(i,:);
            a=a(a>t-10*tau_syn);
            a=a(a<t);
            a=a(a>0);
            if isempty(a)
                ...
                g_sp(i,t)=0;
                for nn=1:length(a)
                    g_sp(i,t)=g_sp(i,t)+exp(-(t-a(nn))/tau_syn);
                end
            end
        end
        ...
        for i=1:l4
            ...
            if ((v_l4(i,t)>=v_th_l4) || (rand<=1-exp(-1*fs*dt)))
                if stop_l4(i,1)==0
                    for j=i+n2:i+n2+2*n1
                        up_lim=0.6;
