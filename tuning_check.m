d1=pwd;
cd('J:\adarsh_model\results\testing_data');
load('17l4_ss_rescale.mat');
cd(d1);
sp_l4_inh=wt_sp_l4_inh(1,end).wt;
sp_l4_ex=wt_sp_l4_ex(1,end).wt;
thal_l4_inh=wt_thal_l4_inh(1,end).wt;
thal_l4_ex=wt_thal_l4_ex(1,end).wt;
clearvars -except thal_sp sp_l4_inh sp_l4_ex thal_l4_inh thal_l4_ex l4_inh_ex f1 f2
global sp n1 n2 n3
sp=33;
n1=4;
n2=4;
...
reps=5;
t_Sim=step*5;% in seconds ; total simulation time seconds (to be put in same units)
...
peak=1;
lf_fr_rate=[];
for step_num=step:step:t_Sim
    %fr=fire_oddball_seq(f1,f2,token_len,iti,tun_wid,step,dt);
    fr=50*exp(-(([1:thal]-f1).^2)/(2*tun_wid^2));
   %fr=fire_allfreq_seq(token_len,iti,tun_wid,step,dt);
    ...
    for t=2:step/dt
        for i=1:l4_ex
            if l4_ex_last_spk(i,1)==0
                v_l4_ex(i,t)=(1-lps)*((squeeze(x_thal(i+n2+n3:i+2*n1+n2+n3,2,t-1))'.*thal_l4_ex(i,i:i+2*n1))*g_thal(i+n2+n3:i+n2+n3+2*n1,t-1)+(squeeze(x_sp(i+n3:i+n3+2*n2,2,t-1))'.*sp_l4_ex(i,i:i+2*n2))*g_sp(i+n3:i+n3+2*n2,t-1)-(squeeze(x_l4_inh(i:i+2*n3,2,t-1))'.*l4_inh_ex(i,i:i+2*n3))*g_l4_inh(i:i+2*n3,t-1));
            else
                if t-l4_ex_last_spk(i,1)<=ref_length
                    v_l4_ex(i,t)=(1-lps)*((squeeze(x_thal(i+n2+n3:i+2*n1+n2+n3,2,t-1))'.*thal_l4_ex(i,i:i+2*n1))*g_thal(i+n2+n3:i+n2+n3+2*n1,t-1)+(squeeze(x_sp(i+n3:i+n3+2*n2,2,t-1))'.*sp_l4_ex(i,i:i+2*n2))*g_sp(i+n3:i+n3+2*n2,t-1)-(squeeze(x_l4_inh(i:i+2*n3,2,t-1))'.*l4_inh_ex(i,i:i+2*n3))*g_l4_inh(i:i+2*n3,t-1))-beta*exp(-(t-l4_ex_last_spk(i,1)-1)/t_refr);
                else
                    v_l4_ex(i,t)=(1-lps)*((squeeze(x_thal(i+n2+n3:i+2*n1+n2+n3,2,t-1))'.*thal_l4_ex(i,i:i+2*n1))*g_thal(i+n2+n3:i+n2+n3+2*n1,t-1)+(squeeze(x_sp(i+n3:i+n3+2*n2,2,t-1))'.*sp_l4_ex(i,i:i+2*n2))*g_sp(i+n3:i+n3+2*n2,t-1)-(squeeze(x_l4_inh(i:i+2*n3,2,t-1))'.*l4_inh_ex(i,i:i+2*n3))*g_l4_inh(i:i+2*n3,t-1));
                end
            end
    ...
    for i=1:thal
        a=thal_fire_time(i,:);
        a=a(a~=0);
        thal_fr(i,iter_num)=length(a);
    end
     for i=1:sp
        a=sp_fire_time(i,:);
        a=a(a~=0);
        sp_fr(i,iter_num)=length(a);
     end
     for i=1:l4_inh
        a=l4_inh_fire_time(i,:);
        a=a(a~=0);
        l4_inh_fr(i,iter_num)=length(a);
     end
     for i=1:l4_ex
        a=l4_ex_fire_time(i,:);
        a=a(a~=0);
        l4_ex_fr(i,iter_num)=length(a);
     end
    iter_num=iter_num+1;
end
